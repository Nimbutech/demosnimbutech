...
class AccountMove(models.Model):
    ...
    url_pdf = fields.Char(string='Representación grafica', tracking=True, copy=False)
    url_xml = fields.Char(string='Archivo XML', tracking=True, copy=False)
    invoice_status_dian = fields.Selection(selection=[('Exitoso', 'Exitoso'), ('Fallida', 'Fallida')],
                                           string='Estado de la factura DIAN', copy=False, tracking=True,
                                           readonly=True)
    ...
    def obtener_resolucion_encabezado(self, move, invoice_type):
        if invoice_type == '91':
            ...
            return {'InvoiceRef': move.debit_origin_id.name,
                    ...
                    'InvoiceDateRef': move.debit_origin_id.invoice_date.strftime('%d/%m/%Y') if move.debit_origin_id.name else "0",
                    'DueDateRef': move.debit_origin_id.invoice_date_due.strftime('%d/%m/%Y') if move.debit_origin_id.name else "0",
                    'CMReasonCode_c': '0',
                    'CMReasonDesc_c': '0',
            ...
            return {'InvoiceRef': '0',
                    ...
                    'DMReasonCode_c': '0',
                    'DMReasonDesc_c': '0',
                    'CalculationRate_c': str(round(rate,2)) if move.currency_id.name != 'COP' else '0',
                    'DateCalculationRate_c': inv_date.strftime('%Y-%m-%d') if move.currency_id.name != 'COP' else '0',
                    'Resolution_str': move.journal_id.sequence_id.resolution_id.resolution_resolution,
                    'ResolutionPrefix': move.journal_id.sequence_id.prefix,
                    'ResolutionDateInvoice': move.journal_id.sequence_id.resolution_id.resolution_resolution_date.strftime(
                        '%Y-%m-%d'),
            ...
            if responsews.status_code == 200:
                ...
                if respuestaws == 'PROCESADO_CORRECTAMENTE':
                    ...
                    try:
                        ...
                        zf.write(directoriozip + move.name.replace("/", "_") + '.pdf', compress_type=compression,
                                 arcname=os.path.basename(directoriozip + move.name.replace("/", "_") + '.pdf'))
                    finally:
                        zf.close()
    ...
    def GetNitCompany(self, number):
        document = ''
        try:
            if '-' in number and self.partner_id.l10n_latam_identification_type_id.l10n_co_document_code != 'external_id':
